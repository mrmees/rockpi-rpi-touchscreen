...
if grep -Fq '`' "$touch_mapper"; then
	die 'touch mapper uses unsupported legacy backtick substitution; use $(...)'
fi
if ! awk '
	...
	function is_redirection_operator(token) {
		return token ~ /^[0-9]*(<|>|<<|>>|<>|<&|>&|[>][|]|<<-)$/
	}
	...
	function begin_substitution(resume_quote) {
		if (need_xrandr_argument)
			reject_xrandr()
		substitution_depth++
		outer_word[substitution_depth] = word
		outer_command_position[substitution_depth] = command_position
		outer_redirection_target[substitution_depth] = redirection_target
		resume_double_quote[substitution_depth] = resume_quote
		word = ""
		have_word = 0
		command_position = 1
		redirection_target = 0
	...
	function end_substitution(    depth) {
		finish_word()
		if (need_xrandr_argument)
			reject_xrandr()
		depth = substitution_depth
		word = outer_word[depth] "__command_substitution__"
		have_word = 1
		command_position = outer_command_position[depth]
		redirection_target = outer_redirection_target[depth]
		if (resume_double_quote[depth])
			double_quoted = 1
		delete outer_word[depth]
		delete outer_command_position[depth]
		delete outer_redirection_target[depth]
		delete resume_double_quote[depth]
		substitution_depth--
	}
