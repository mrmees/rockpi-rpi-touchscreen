...
install)
	...
	if [ "$version" = 0.2.1 ] && [ "${DKMS_FAIL_OLD_REINSTALL:-0}" -eq 1 ]; then
		exit 26
	fi
	...
	if [ "$version" = 0.2.1 ] || [ "$version" = 0.2.2 ]; then
		...
		if [ "$version" = 0.2.2 ] &&
			[ "${DKMS_FAIL_INSTALL_MODULE:-}" = panel_rockpi_rpi_touchscreen ]; then
			exit 25
		fi
	fi
	if [ "${DKMS_NEW_STATUS_PHASE:-}" = built ] && [ "$version" = 0.2.2 ]; then
		exit 0
	fi
...
remove)
	...
	if [ "${DKMS_REQUIRE_NEW_STATE_BEFORE_OLD_REMOVE:-0}" -eq 1 ] && [ "$version" = 0.2.1 ]; then
		...
		[ -f "${DKMS_TREE:?}/rockpi-rpi-touchscreen-0.2.2/src/panel_rockpi_rpi_touchscreen.c" ] || exit 35
		[ -f "${BACKUP_PATH:?}" ] && [ -f "${BACKUP_PATH:?}.sha256" ] || exit 36
		sha256sum -c "${BACKUP_PATH:?}.sha256" >/dev/null || exit 37
		cmp "${DKMS_STATE_DIR:?}/rockpi-rpi-touchscreen/0.2.2/test-kernel/aarch64/module/raspits_ft5426.ko" \
			"${MODULES_DIR:?}/test-kernel/updates/dkms/raspits_ft5426.ko" || exit 33
		cmp "${DKMS_STATE_DIR:?}/rockpi-rpi-touchscreen/0.2.2/test-kernel/aarch64/module/panel_rockpi_rpi_touchscreen.ko" \
			"${MODULES_DIR:?}/test-kernel/updates/dkms/panel_rockpi_rpi_touchscreen.ko" || exit 34
		[ "$(cat "${DKMS_ACTIVE_STATE:?}")" = 'test-kernel|0.2.2' ] || exit 38
		if [ -f "${DKMS_INSTALLED_STATE:?}" ] &&
			grep -Fxq '0.2.1|test-kernel|aarch64' "$DKMS_INSTALLED_STATE"; then
			exit 39
		fi
...
test_offline_boot_rollback_changes_only_explicit_target_root()
{
	...
	mkdir -p "$target_root/boot" "$sandbox/host-usr-src/rockpi-rpi-touchscreen-0.2.2"
	printf '%s\n' 'user_overlays=spi-test rockpi-4b-plus-rpi-touchscreen' > "$target_root/boot/armbianEnv.txt"
	: > "$sandbox/host-usr-src/rockpi-rpi-touchscreen-0.2.2/sentinel"
	: > "$sandbox/dkms.log"
	...
	[ -f "$sandbox/host-usr-src/rockpi-rpi-touchscreen-0.2.2/sentinel" ] ||
...
extraargs=console=ttyS2'
	...
	[ -f "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2/dkms.conf" ] || \
		fail 'installer must copy owned DKMS source tree'
	assert_module_matches_build "$sandbox" 0.2.2 raspits_ft5426
	assert_module_matches_build "$sandbox" 0.2.2 panel_rockpi_rpi_touchscreen
...
test_uninstall_removes_only_project_token_and_dry_run_is_scoped()
{
	...
	printf '%s\n' "$dry_run" | grep -Fqx "REMOVE: $sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2" ||
...
extraargs=console=ttyS2' \
	...
	assert_file_absent "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2"
...
test_same_version_changed_source_is_rejected()
{
	...
	make_sandbox "$sandbox"
	run_install "$sandbox" "$sandbox/validate-pass.sh"
	source_file=$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2/scripts/dkms-make.sh
	printf '\n# changed source\n' >> "$source_file"
...
test_dkms_source_package_is_allowlisted()
{
	...
	make_sandbox "$sandbox"
	run_install "$sandbox" "$sandbox/validate-pass.sh"
	source=$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2
...
test_uninstall_dkms_failure_retains_source_and_fails()
{
	...
	[ -d "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2" ] ||
...
test_uninstall_dkms_status_failure_leaves_all_assets()
{
	...
	dtbo=$sandbox/boot/overlay-user/rockpi-4b-plus-rpi-touchscreen.dtbo
	dtbo_before=$(sha256sum "$dtbo" | awk '{print $1}')
	if DKMS_STATUS_FAIL_VERSION=0.2.2 run_uninstall "$sandbox" > "$sandbox/output" 2>&1; then
		fail 'uninstall accepted DKMS status failure'
	fi
	[ -d "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2" ] || fail 'status failure removed source'
	assert_equal "$(sha256sum "$sandbox/boot/armbianEnv.txt" | awk '{print $1}')" "$config_before" \
...
assert_failed_uninstall_restored()
{
	...
	assert_equal "$(sandbox_dkms_status "$sandbox" 0.2.2)" \
		'rockpi-rpi-touchscreen/0.2.2, test-kernel, aarch64: installed' \
		'post-removal uninstall failure did not restore exact DKMS lifecycle'
	assert_equal "$(cat "$sandbox/dkms-active.state")" 'test-kernel|0.2.2' \
...
test_uninstall_refuses_unowned_unregistered_source()
{
	...
	source=$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2
	mkdir -p "$source"
	printf '%s\n' 'PACKAGE_NAME="some-other-package"' 'PACKAGE_VERSION="0.2.2"' > "$source/dkms.conf"
...
{
	sandbox=$1
	old=$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1
	mkdir -p "$old"
	printf '%s\n' 'PACKAGE_NAME="rockpi-rpi-touchscreen"' 'PACKAGE_VERSION="0.2.1"' > "$old/dkms.conf"
	printf '%s\n' '0.2.1' > "$sandbox/dkms-added.state"
	printf '%s\n' '0.2.1|test-kernel|aarch64' > "$sandbox/dkms-built.state"
	printf '%s\n' '0.2.1|test-kernel|aarch64' > "$sandbox/dkms-installed.state"
	printf '%s\n' 'test-kernel|0.2.1' > "$sandbox/dkms-active.state"
	mkdir -p "$sandbox/var-lib-dkms/rockpi-rpi-touchscreen/0.2.1/test-kernel/aarch64/module" \
		"$sandbox/modules/test-kernel/updates/dkms"
	printf '%s\n' 'old-touch-module' > \
		"$sandbox/var-lib-dkms/rockpi-rpi-touchscreen/0.2.1/test-kernel/aarch64/module/raspits_ft5426.ko"
	printf '%s\n' 'old-panel-module' > \
		"$sandbox/var-lib-dkms/rockpi-rpi-touchscreen/0.2.1/test-kernel/aarch64/module/panel_rockpi_rpi_touchscreen.ko"
	cp "$sandbox/var-lib-dkms/rockpi-rpi-touchscreen/0.2.1/test-kernel/aarch64/module/raspits_ft5426.ko" \
		"$sandbox/modules/test-kernel/updates/dkms/raspits_ft5426.ko"
	cp "$sandbox/var-lib-dkms/rockpi-rpi-touchscreen/0.2.1/test-kernel/aarch64/module/panel_rockpi_rpi_touchscreen.ko" \
...
test_migration_removes_old_release_only_after_success()
{
	...
	assert_file_absent "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1"
	assert_equal "$(sandbox_dkms_status "$sandbox" 0.2.2)" \
		'rockpi-rpi-touchscreen/0.2.2, test-kernel, aarch64: installed' \
		'new DKMS version did not reach exact installed state'
	assert_equal "$(sandbox_dkms_status "$sandbox" 0.2.1)" '' \
	...
	assert_module_matches_build "$sandbox" 0.2.2 raspits_ft5426
	assert_module_matches_build "$sandbox" 0.2.2 panel_rockpi_rpi_touchscreen
	printf 'PASS: successful 0.2.1 to 0.2.2 migration after complete verification\n'
}
...
test_failed_migration_retains_old_release()
{
	...
	[ -f "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1/dkms.conf" ] ||
		fail 'failed migration removed old source'
	assert_equal "$(sandbox_dkms_status "$sandbox" 0.2.1)" \
		'rockpi-rpi-touchscreen/0.2.1, test-kernel, aarch64: installed' \
		'failed migration did not retain exact old installed state'
	assert_file_absent "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2"
	printf 'PASS: failed migration retains 0.2.1 release\n'
}
...
test_install_add_mutate_then_fail_restores_absent_baseline()
{
	...
	grep -Fqx 'remove -m rockpi-rpi-touchscreen -v 0.2.2 --all' "$sandbox/dkms.log" ||
...
test_incomplete_snapshot_leaves_old_modules_untouched()
{
	...
	assert_equal "$(cat "$sandbox/dkms-active.state")" 'test-kernel|0.2.1' \
		'incomplete snapshot rollback changed active version'
	assert_module_matches_build "$sandbox" 0.2.1 panel_rockpi_rpi_touchscreen
	assert_file_absent "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2"
...
test_second_module_install_failure_restores_old_release_and_boot()
{
	...
	if grep -Eq 'rollback also failed|recovery artifacts retained' "$sandbox/output"; then
		fail 'second-module failure did not complete a clean old-version rollback'
	fi
	[ -f "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1/dkms.conf" ] ||
	...
	grep -Fqx 'install -m rockpi-rpi-touchscreen -v 0.2.1 -k test-kernel' "$sandbox/dkms.log" ||
	...
	assert_module_matches_build "$sandbox" 0.2.1 raspits_ft5426
	assert_module_matches_build "$sandbox" 0.2.1 panel_rockpi_rpi_touchscreen
	...
	printf 'PASS: second-module failure restores 0.2.1 modules and preserves boot state\n'
}
...
test_failed_new_registration_removal_retains_recovery_source()
{
	...
	[ -n "$(sandbox_dkms_status "$sandbox" 0.2.2)" ] || fail 'test did not retain failed new registration'
	grep -Fq "new source retained at $sandbox/usr-src/rockpi-rpi-touchscreen-0.2.2" "$output" ||
...
test_failed_old_reinstall_reports_preserved_recovery()
{
	...
	grep -Fq "old DKMS reinstall failed; retained old source at $sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1" "$output" ||
...
test_old_status_failure_retains_source_and_does_not_claim_success()
{
	...
	seed_old_release "$sandbox"
	output=$sandbox/output
	if DKMS_STATUS_FAIL_VERSION=0.2.1 \
		run_install "$sandbox" "$sandbox/validate-pass.sh" > "$output" 2>&1; then
		fail 'installer accepted unverifiable old DKMS state'
	fi
	[ -f "$sandbox/usr-src/rockpi-rpi-touchscreen-0.2.1/dkms.conf" ] ||
		fail 'status failure removed old source'
	assert_equal "$(DKMS_STATUS_FAIL_VERSION= sandbox_dkms_status "$sandbox" 0.2.1)" \
		'rockpi-rpi-touchscreen/0.2.1, test-kernel, aarch64: installed' \
