...
require_text README.md 'DKMS `0.2.5` installs exactly three production modules in provider, panel, then touch order:'
require_text README.md '`rockpi_rk3399_display_compat`, `panel_rockpi_rpi_touchscreen`, and `raspits_ft5426`'
require_text README.md 'The exact migration baseline is DKMS `0.2.4` with the same three modules in that order.'
